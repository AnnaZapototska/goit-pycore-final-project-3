...
from bot import add_birthday, add_contact, all_command, change_command, change_email_command, phone_command, show_birthday
...
class ContactIdentityTests(unittest.TestCase):
    ...
    def test_can_add_two_contacts_with_same_name_and_different_primary_phones(self):
        first_result = add_contact(["Alice", "0123456789", "alice.one@example.com"], self.book)
        second_result = add_contact(["Alice", "1234567890", "alice.two@example.com"], self.book)
        ...
        self.assertEqual(self.book.find("0123456789").email.value, "alice.one@example.com")
        self.assertEqual(self.book.find("1234567890").email.value, "alice.two@example.com")
    ...
    def test_duplicate_primary_phone_is_treated_as_same_contact_update(self):
        ...
        self.assertEqual(self.book.find("0123456789").name.value, "Alice Updated")
        self.assertEqual(len(list(self.book.iter_records())), 1)

    def test_duplicate_email_is_rejected(self):
        add_contact(["Alice", "0123456789", "shared@example.com"], self.book)

        result = add_contact(["Bob", "1234567890", "shared@example.com"], self.book)
    ...
    def test_all_shows_contacts_with_same_name_as_separate_records(self):
        add_contact(["Alice", "0123456789", "alice.one@example.com"], self.book)
        add_contact(["Alice", "1234567890", "alice.two@example.com"], self.book)
    ...
    def test_change_email_uses_phone_selector(self):
        ...
        result = change_email_command(["1234567890", "alice.two@example.com"], self.book)
    ...
    def test_phone_and_birthday_commands_use_phone_or_email_selector(self):
        ...
        birthday_result = add_birthday(["alice@example.com", "01.01.2000"], self.book)
        ...
        self.assertEqual(phone_result, "Alice's primary phone number is 0123456789.")
        self.assertEqual(show_birthday_result, "Alice's birthday is on 2000-01-01.")
    ...
    def test_change_updates_primary_phone(self):
        ...
        self.assertEqual(self.book.find("1234567890").email.value, "alice@example.com")
